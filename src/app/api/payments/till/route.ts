...
export async function GET(req: Request) {
  ...
  // - range: inclusive daily window from `from` → `to` in APP_TZ
    ...
    const rows = (raw || []).map((r: any) => ({
      ...
      // Unified reference, prefer mpesaReceipt then request ids then account reference
      ref: r.mpesaReceipt || r.checkoutRequestId || r.merchantRequestId || r.accountReference || null,
      reference: r.mpesaReceipt || r.checkoutRequestId || r.merchantRequestId || r.accountReference || null,
