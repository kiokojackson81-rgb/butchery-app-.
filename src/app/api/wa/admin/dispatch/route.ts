...
// Admin-only: simulate inbound text or button for a given phone in production.
// Requires header: x-admin-secret: <ADMIN_API_SECRET>
// Body: { phoneE164: "+2547...", text?: string, buttonId?: string }
export async function POST(req: Request) {
  try {
    ...
    if (text) {
      ...
      return NextResponse.json({ ok: true, routed: role, kind: "text" });
    }
    if (buttonId) {
      ...
      return NextResponse.json({ ok: true, routed: role, kind: "button" });
    }
    return NextResponse.json({ ok: false, error: "text or buttonId required" }, { status: 400 });
  } catch (e: any) {
    return NextResponse.json({ ok: false, error: e?.message || "dispatch failed" }, { status: 500 });
  }
}
