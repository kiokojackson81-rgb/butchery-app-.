...
type Body = { id: string; closingQty?: number; wasteQty?: number };
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      await (prisma as any).setting.create({ data: { key, value: { type: 'closing', id, at: new Date().toISOString(), before, after } } });
    } catch {}
