...
export async function POST(req: Request) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (from?.lockedAt) {
        const err: any = new Error("from_locked");
        err.code = "FROM_LOCKED";
        throw err;
      }
      ...
      if (to?.lockedAt) {
        const err: any = new Error("to_locked");
        err.code = "TO_LOCKED";
        throw err;
      }
    ...
    if (err?.code === "FROM_LOCKED") {
      return NextResponse.json({ ok: false, error: "from_locked" }, { status: 409 });
    }
    if (err?.code === "TO_LOCKED") {
      return NextResponse.json({ ok: false, error: "to_locked" }, { status: 409 });
    }
    throw err;
  }
