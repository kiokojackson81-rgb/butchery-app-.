  ...
  const supplyByItem: Record<string, number> = {};
      ...
      if (nextCount === 1) {
        ...
        try {
          ...
          // Aggregate till sales gross for the just-closed in-day period (payments since activePeriod start until now)
          let tillSalesGross = 0;
          try {
            const active = await (tx as any).activePeriod.findFirst({ where: { outletName: outlet } }).catch(()=>null);
            const startBound = active?.periodStartAt ? new Date(active.periodStartAt) : new Date(date + "T00:00:00.000Z");
            const endBound = new Date();
            // Map outlet human label -> Payment.outletCode enum (reuse logic from metrics/header)
            const allowedCodes = ["BRIGHT","BARAKA_A","BARAKA_B","BARAKA_C","GENERAL"] as const;
            const toEnum = (s: string | null | undefined) => {
              if (!s) return null as any; const c = String(s).trim().toUpperCase().replace(/[^A-Z0-9]+/g, "_");
              return (allowedCodes as readonly string[]).includes(c) ? c : null;
            };
            ...
            if (outletEnum) {
              const agg = await (tx as any).payment.aggregate({ where: { outletCode: outletEnum, status: 'SUCCESS', createdAt: { gte: startBound, lte: endBound } }, _sum: { amount: true } });
