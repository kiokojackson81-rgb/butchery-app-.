...
export async function saveClosings(args: {
  ...
  // Case-insensitive by itemKey to avoid mismatches
  ...
  try {
    ...
    for (const r of prev || []) {
      ...
      openEffMap.set(k, (openEffMap.get(k) || 0) + q);
    }
  ...
  await (prisma as any).$transaction(async (tx: any) => {
    for (const r of rows) {
      ...
      const itemKeyLc = String(itemKey || '').toLowerCase();
      const openEff = Number(openEffMap.get(itemKeyLc) || 0);
