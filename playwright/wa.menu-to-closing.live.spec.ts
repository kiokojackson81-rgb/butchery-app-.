...
test.describe('Live: menu -> ATT_CLOSING via webhook', () => {
  ...
  test('finalize login, send ATT_CLOSING, observe interactive reply', async ({ request }) => {
    const phone = TEST_WA_E164;
    const phoneDigits = toDigits(phone);

    // 1) Finalize login via login-link API (bind phone ↔ code and send welcome/menu)
    ...
    // 2) Send webhook interactive reply for ATT_CLOSING
    const body = {
      ...
      entry: [{ changes: [{ value: { messages: [{
        ...
        from: phoneDigits,
        type: 'interactive',
        interactive: { type: 'button', button_reply: { id: 'ATT_CLOSING', title: 'Enter Closing' } },
      }] } }] }],
    } as const;
    ...
    // 3) Poll logs for a recent outbound interactive/text response indicating the closing flow started
    const found = await waitFor(async () => {
      ...
      for (const r of rows) {
        ...
        // Accept either interactive (list of products) or an instructive text.
