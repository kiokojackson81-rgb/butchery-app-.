...
$u = "$BaseUrl/api/admin/payments/by-receipt?receipt=$([Uri]::EscapeDataString($Receipt))"
$headers = @{ 'x-admin-key' = $AdminKey }

try {
  $res = Invoke-WebRequest -Method GET -Uri $u -Headers $headers -SkipHttpErrorCheck
  if ($res.StatusCode -ge 200 -and $res.StatusCode -lt 300) {
    try { ($res.Content | ConvertFrom-Json) | ConvertTo-Json -Depth 12 } catch { Write-Host $res.Content }
  } else {
